class RailStat < ActiveRecord::Base
   ...
   # Method returns paths hash with to 40 paths wvisith value = top index (1..40)
   ...
   def RailStat.find_all_by_flag(include_search_engine, number_visits, subdomain)
      if subdomain.nil?
        find(:all, :order => "created_at desc", :limit => number_visits)
      else
        find(:all, :conditions => ["subdomain = ?", subdomain], :order => "created_at desc", :limit => number_visits)
      end
   end
   ...
   #
   # examples:
   #   RailStat.count_visits : returns stats for all visits
   #   RailStat.count_visits(:unique) : returns stats for only unique IPs
   #   RailStat.count_visits(:date => d) : returns stats for the date 'd'
   #   RailStat.count_visits(:unique, :subdomain => "mydomain", :date => d) : returns stats for unique IPs accessing the specified subdomain on the date 'd'
   ###

   def RailStat.count_visits(*params)
      ...
      query << " as visits from rail_stats"
      ...
      find_by_sql(query)[0]['visits'].to_i
   ...
   ##
   #  find_by_days: Find the visits in the specified previous X days. Default is 7 days.
   ##
   def RailStat.find_by_days(params = {})
      params[:days] = 7 unless params[:days]
      query = "SELECT created_on, COUNT(*) AS total_visits, COUNT(DISTINCT remote_ip) AS unique_visits FROM rail_stats WHERE created_on >= '#{(Time.now - params[:days]*24*60*60).strftime("%Y-%m-%d")}'"
   ...
   ##
   #  find_first_visit: What is this used for?
   ##
   def RailStat.find_first_visit(params = {})
