// Graticule (google.maps v3); adapted from www.bdcc.co.uk Bill Chadwick 2006
// Free for any use
...
Graticule.prototype = new google.maps.OverlayView();
Graticule.prototype.mapPane = function () {
  return this.getPanes().overlayShadow;
};
Graticule.prototype.latLngToPixel = function(lat, lng) {
...
Graticule.prototype.onAdd = function () {
  var self = this;
  this.maptypeear_ = google.maps.event.addListener(this.getMap(),
    'maptypeid_changed', function () {
      self.draw();
    }
  );
};
Graticule.prototype.onRemove = function() {
  this.remove();
  if (this.maptypeear_) {
    google.maps.event.removeListener(this.maptypeear_);
  }
};
...
Graticule.prototype.draw = function() {
  ...
  this.color_ = this._bestTextColor();

  // determine graticule interval
  var bnds = this.getMap().getBounds();
  if (!bnds) {
    return; // The map is not ready yet.
  }
...
Graticule.prototype.createVLine = function(x) {
  return this.createLine(x,this.minY,"1",(this.maxY-this.minY));
};
Graticule.prototype.createHLine = function(y) {
  return this.createLine(0,y,this.getMap().getSize().width,"1");
};

//returns a div that is a horizontal single pixel line, across the dateline  
//we find the start and width of a 180 degree line and draw the same amount to
//its left and right	  
