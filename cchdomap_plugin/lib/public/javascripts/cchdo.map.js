...
CM.pane = (function () {
  ...
  _.redraw = function () {
    ...
    GM.event.trigger(CM.map, 'resize');
...
//      }
//      SelectableTrack.prototype = new GM.OverlayView();
...
CM.load = function () {
  var opts = {
    ...
    mapTypeControlOptions: {
      ...
      mapTypeIds: [GM.MapTypeId.TERRAIN, GM.MapTypeId.SATELLITE,
                   GM.MapTypeId.HYBRID, GM.MapTypeId.ROADMAP]
    }
  };
  CM.map = new GM.Map($('#map')[0], opts);
  ...
  GM.event.addListener(CM.earth, 'initialized', function () {
    CM.earth.set('graticules', true);
  });
  ...
  $('#gridon').change(function () { CM.setGrid(CM.gridOn()); }).change();
  $('#atmosphereon').change(function () {
    CM.earth.set('atmosphere', $(this).is(':checked'));
  }).change();
