...
// google.maps.StreetViewPanorama, and google.maps.Marker collaborate to allow
// markers to be duplicated in both Map and StreetView.
//
...
var Linker = (function () {
  ...
  _.prototype.getId = function (obj) {
    if (!obj[this.id_name]) {
  ...
  _.prototype.likelyNotUserMarker = function (obj) {
  ...
  _.prototype.remove = function (obj) {
    var k = this.getId(obj);
    if (this.map[k]) {
      delete this.map[k];
  ...
  _.prototype.contains = function (obj) {
    var k = this.getId(obj);
    if (this.map[k]) {
      return true;
    }
  ...
  _.prototype.forEach = function (fn) {
...
// google.earth
function MapOverlayToEarth(earth) {
  var self = this;
  this.earth = earth;
...
MapOverlayToEarth.prototype.insert = function (x) {
  var self = this;
  ...
  function doInsert(mapobj, placemark, listeners) {
    ...
    self.earth._withEarth(function (ge) {
...
MapOverlayToEarth.prototype.remove = function (x) {
  var self = this;
  this.earth._withEarth(function (ge) {
    var k = Linker.getId(x);
    var earthobj = self.map_earth[k];
    ...
    self.map_earth[k] = null;
    self.earth_map[placemark] = null;
  });
};
MapOverlayToEarth.prototype.hexColorAndAlphaToEarthColor = function (hex, alpha) {
  if (hex && hex[0] == '#') {
    hex = hex.slice(1);
  }
...
MapOverlayToEarth.prototype.createMarker = function (marker, doInsert) {
  var self = this;
  this.earth._withEarth(function (ge) {
    ...
    if (marker.getIcon()) {
      ...
      var absurl = url;
      if (absurl[0] == '/') {
        absurl = window.location.protocol + '//' + window.location.host + absurl;
      }
      ...
      mapImg.onload = function () {
        ...
        iconStyle.getIconStyle().setScale(Math.min(icoWidth, icoHeight) / 32.0);
...
MapOverlayToEarth.prototype.makePathListener = function (placemark, latlngs,
                                                         coordinates) {
...
MapOverlayToEarth.prototype.createPolyline = function (polyline, doInsert) {
  var self = this;
  this.earth._withEarth(function (ge) {
    var placemark = ge.createPlacemark('');
    ...
    var listeners = self.makePathListener(placemark, latlngs, coordinates);
...
MapOverlayToEarth.prototype.createPolygon = function (polygon, doInsert) {
  var self = this;
  this.earth._withEarth(function (ge) {
    var placemark = ge.createPlacemark('');
...
EarthMapType.prototype._initEarth = function (ge) {
  ...
  this.mapper = new MapOverlayToEarth(this);
