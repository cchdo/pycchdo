...
module PathTracker
  ...
  def track_path
    begin
      env = request.env
      env ||= {'HTTP_USER_AGENT' => nil, 'HTTP_REFERER' => nil, 'REMOTE_ADDR' => nil, 'HTTP_ACCEPT_LANGUAGE' => nil, 'REQUEST_URI' => nil}

      referer = params['referer']
      url = env['HTTP_REFERER']
      
      req_uri = get_doc_url(url)
      
      user_agent = parse_user_agent(env['HTTP_USER_AGENT'])
      ...
      unless req_uri == '/'
        parameters = url[url.index(req_uri) + req_uri.length, url.length]
      else
        parameters = ""
      end
      ...
      RailStat.create(
        ...
        "screen_size" => params['size'],
        "colors" => params['colors'],
        "java" => params['java'],
        "java_enabled" => params['je'],
        "flash" => params['flash'],
        "parameters" => parameters
      )
    rescue Exception => e
      RAILS_DEFAULT_LOGGER.error("Error in path_tracker: #{e}" )
    end                 
  end
  ...
  def get_urls_host(ref)
    if host = parse_url(ref) and not host == SITE_NAME
      return host
    end
    return nil
  end
  ...
  def parse_url(arg)
    unless arg.nil?
      return URI::split(arg.gsub(' ','%20'))[2] 
    end
    return nil
  end
  ...
  def get_doc_url(arg)
    unless arg.nil?
      return URI::split(arg.gsub(' ','%20'))[5] 
    end
    return nil
  end
  ...
  def get_params_url(arg)
    unless arg.nil?
      return URI::split(arg.gsub(' ','%20'))[7] 
    end
    return nil
  end
  ...
  def detect_subdomain
    if request and request.subdomains and first = request.subdomains.first
      return first
    end
    return nil
  end
